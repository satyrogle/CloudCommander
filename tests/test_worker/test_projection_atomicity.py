...
async def test_replayed_event_is_idempotent_for_projection(db_pool, reset_db):
    ...
    event_hash = generate_event_hash(None, payload, timestamp_utc_ms, 2)
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(
            ...
            INSERT INTO events (
                ...
                idempotency_key, actor_id, actor_claims, expected_version, event_type,
                payload, previous_hash, event_hash
            ) VALUES ($1, $2, $3, 2, $4, 'idem-replay', 'operator-1', $5::jsonb, 1, 'ResourceAllocationRequested', $6::jsonb, NULL, $7)
            """,
            event_id,
            tenant_id,
            node_id,
